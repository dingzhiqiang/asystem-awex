...
"""CPU contract tests for the Qwen3-MoE SGLang weight converter."""
...
def test_convert_param_expert_names_use_global_ids_with_ep():
    ...
    converted_names = set()
    for name, param in _sglang_named_params(num_local_experts=NUM_EXPERTS // 2).items():
        for hf_name, _ in converter.convert_param(name, param):
            converted_names.add(hf_name)
    assert converted_names == _expected_hf_names(range(NUM_EXPERTS // 2, NUM_EXPERTS))
