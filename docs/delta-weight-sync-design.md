...
### 4.6 共卡模式 Delta 完整设计 (重点)

共卡模式是 delta 实施的**最复杂场景**，因为 SGLang 权重在训练期间被释放（TMS pause），delta 的"在旧权重上打补丁"前提需要特殊处理。

#### 4.6.1 核心约束：Delta 需要"旧权重"作为 base

Delta 的本质是 `new_weights = old_weights + sparse_patch`。接收方（SGLang）必须持有上一版本的权重才能 apply patch。

在共卡模式中，SGLang 权重经历了：
```
pause("weights") → GPU 显存释放 → 训练占用 GPU → resume("weights") → ???
```

`resume("weights")` 后的状态取决于 TMS 配置：

| TMS `enable_cpu_backup` | pause 时 | resume 后 | Delta 可用? |
|------------------------|---------|----------|-----------|
| `True` | weights → CPU backup → GPU 释放 | 从 CPU 恢复旧权重 | ✅ |
| `False` | 直接释放 GPU（weights 丢失） | 空 storage（垃圾值） | ❌ 必须全量 |

**结论**：共卡 + delta 要求 `TMS_INIT_ENABLE_CPU_BACKUP=1`。

#### 4.6.2 完整时序（共卡 + Delta）

```
                Actor (训练进程)                      SGLang (推理进程)
                ──────────────                      ───────────────

Step 0 (anchor，首次全量传输):
                                                    weights 在 GPU ✓
    ┌─ Actor offload optimizer                      │
    ├─ 全量 IPC serialize → MetaServer              │
    │                                               ├─ IPC deserialize → 写入模型
    │                                               ├─ TMS 将 weights 纳入管理 region
    └─ Actor offload weights                        └─ resume kv_cache, 开始 rollout

Rollout 完成:
                                                    pause("weights","kv_cache","cuda_graph")
                                                      → weights CPU backup ← 这是 delta 的 base
                                                      → kv_cache 释放
                                                      → cuda_graph 释放

Step N (delta 传输):
    ┌─ Actor onload weights + optimizer
    ├─ Actor 训练 (forward + backward + optimizer step)
    ├─ compute_delta(model) → DeltaResult (1-2% sparsity)
    │     比较: 当前训练权重 vs detector snapshot
    ├─ Actor offload optimizer
    ├─ delta IPC serialize → MetaServer               
    │     只传: {name: (indices_int32, values_bf16)}
    │     大小: ~200MB vs 全量 14GB (7B model)
    │                                               ├─ resume("weights") ← TMS 从 CPU 恢复旧权重
    │                                               ├─ MetaServer get delta
    │                                               ├─ IPC deserialize delta
    │                                               ├─ scatter_(indices, values) ← 就地打补丁
    │                                               │     旧权重 + delta = 新权重 ✓
    │                                               ├─ NCCL reshard (跨 PP 层，发 full tensor)
    │                                               ├─ 通知 Actor "done"
    ├─ Actor 收到 "done"
    ├─ Actor 释放 IPC shared tensor
    ├─ Actor offload weights
    │                                               ├─ resume("kv_cache","cuda_graph")
    │                                               └─ 开始 rollout
    └─ detector.update_snapshot (记录当前版本作为下次 base)
```

#### 4.6.3 为什么 IPC delta 对共卡仍有收益

共卡 IPC 是同卡零拷贝，带宽不是瓶颈。但 delta 仍然有意义：

| 环节 | 全量 (7B) | Delta (2% sparsity) | 节省 |
|------|----------|--------------------|----- |
| group_tensors_by_shape_and_dtype | 处理 ~200 个参数 | 跳过无变化的 ~196 个参数 | 98% compute |
| share_memory_() 分配 | ~14 GB shared buffer | ~200 MB shared buffer | 98% memory |
| IPC serialize | 序列化 ~14 GB | 序列化 ~200 MB | 98% latency |
| MetaServer put | 传 ~14 GB 句柄 | 传 ~200 MB 句柄 | 句柄数减少 |
| SGLang 侧 copy | memcpy 14 GB | scatter 200MB (random access) | 看 scatter 效率 |

**关键节省**：不是网络带宽，而是 **GPU 显存分配和 serialize 时间**。全量路径需要在 GPU 上分配 14GB 的 share_memory buffer（IPC 共享），delta 只需要 200MB。

#### 4.6.4 SGLang 侧的 scatter_ 性能

`param.data.flatten().scatter_(0, indices, values)` 是 random access write，比 sequential copy 慢。

但对于 2% sparsity（7B 模型 ~70M 个变化元素）：
- 70M 个 scatter write，每个 2 bytes (bf16) = 140MB 随机写
- GPU global memory bandwidth ~2TB/s
- 理论时间: ~0.07ms（忽略 cache miss）
- 实际：考虑 random access pattern，约 1-5ms

vs 全量 copy：14GB / 2TB/s = 7ms

**结论**：scatter 性能在可接受范围内。

#### 4.6.5 共卡 + PP 不同时的 NCCL Reshard

场景：训练 PP=2，推理 PP=1。每个推理 rank 需要全部层，但同卡训练 rank 只有部分层。

```
GPU 0: Train PP=0 (layers 0-15) + Infer rank 0 (需要 layers 0-31)
GPU 4: Train PP=1 (layers 16-31) + Infer rank 4 (需要 layers 0-31)
```

Delta 流程：
```
Infer rank 0:
  ① resume("weights") → 恢复旧的 layers 0-31 (全部层)
  ② IPC 收到 layers 0-15 的 delta → scatter apply (layers 0-15 现在是最新)
  ③ NCCL send: layers 0-15 的 full tensor → Infer rank 4
  ④ NCCL recv: layers 16-31 的 full tensor ← Infer rank 4
  ⑤ 覆盖 layers 16-31 (因为是从对方收到的最新版)

Infer rank 4:
  ① resume("weights") → 恢复旧的 layers 0-31
  ② IPC 收到 layers 16-31 的 delta → scatter apply
  ③ NCCL send: layers 16-31 的 full tensor → Infer rank 0
  ④ NCCL recv: layers 0-15 的 full tensor ← Infer rank 0
  ⑤ 覆盖 layers 0-15
```

**关键细节**：NCCL reshard 传的是 apply delta 后的 **full tensor**（不是 delta）。因为：
1. 接收方在 resume 后有旧权重，但它的"旧权重"和发送方 apply delta 后的"新权重"是不同版本——直接 delta 无法对齐
2. NCCL reshard 跨卡，数据量是 1/PP × model_size（PP=2 时 7GB），NVLink 600GB/s 下 ~12ms
3. 实现简单，复用现有 recursive partition 算法不改

#### 4.6.6 共卡 + TP 不同时

场景：训练 TP=4，推理 TP=8。一个训练 shard 的 delta 需要拆分给两个推理 shard。

```
Train rank 0 (TP=0): qkv_proj [0:750, :] 有 delta
  → 需要拆分为:
    Infer rank 0 (TP=0): qkv_proj [0:375, :] 的部分 delta
    Infer rank 1 (TP=1): qkv_proj [375:750, :] 的部分 delta
```

流程：
```
① Actor compute_delta() 在 train shard [0:750, :] 上检测变化
② Actor 将 delta 序列化时，附带 shard 信息
③ SGLang rank 0 接收后，用 remap_delta_indices 过滤:
   - 只保留 row < 375 的 indices
   - remap: row 不变（因为 infer shard 从 0 开始）
④ SGLang rank 1 接收后，用 remap_delta_indices 过滤:
   - 只保留 375 <= row < 750 的 indices
   - remap: row -= 375（映射到 infer shard 的 local space）
```

**这里有个选择**：
- **方案 A**：Actor 侧提前按 infer rank 拆分 delta（每个 infer rank 收到自己需要的部分）
- **方案 B**：Actor 发完整 shard delta，每个 infer rank 自己 filter+remap

推荐**方案 B**：实现简单，filter+remap 在 GPU 上 <1ms，不需要 Actor 知道推理侧分片细节。

#### 4.6.7 共卡 Delta 的 fallback 条件

以下情况自动 fallback 到全量传输：

| 条件 | 原因 |
|------|------|
| `version == 0` (首次) | 无旧权重可打补丁 |
| `TMS_INIT_ENABLE_CPU_BACKUP != "1"` | resume 后无旧权重 |
| `delta_size > threshold × full_size` | delta 太大，不如全量 |
| `step % force_full_every_n == 0` | 定期全量防止精度漂移 |
| SGLang 侧检测到权重 checksum 不匹配 | 容错恢复 |

#### 4.6.8 显存时间线（共卡 + Delta, 7B bf16, 80GB GPU）

```
时间 →   |-- rollout --|-- train --|-- weight sync --|-- rollout --|

GPU 占用:
  ┌──────────────────────────────────────────────────────────────────┐
  │ Rollout阶段:                                                     │
  │   weights: 14GB (SGLang model)                                   │
  │   kv_cache: ~55GB                                                │
  │   cuda_graph: ~2GB                                               │
  │   total: ~71GB / 80GB                                            │
  ├──────────────────────────────────────────────────────────────────┤
  │ pause("weights","kv_cache","cuda_graph"):                        │
  │   → weights 14GB → CPU backup (释放 GPU)                         │
  │   → kv_cache 55GB → 释放 (无 backup，每步重建)                    │
  │   → cuda_graph 2GB → 释放                                        │
  │   GPU: ~0 GB (全给训练)                                           │
  ├──────────────────────────────────────────────────────────────────┤
  │ Train阶段:                                                       │
  │   Actor weights: 14GB                                             │
  │   Actor optimizer: 56GB (AdamW, fp32 m+v)                        │
  │   Actor gradients: 14GB                                           │
  │   total: ~84GB → 需要 CPU offload optimizer 才放得下                │
  │   实际: weights 14GB + grad 14GB + partial optim = ~50GB           │
  ├──────────────────────────────────────────────────────────────────┤
  │ Weight sync (Delta):                                              │
  │   Actor offload optimizer: 56GB → CPU                             │
  │   Actor 保留 weights: 14GB (供 delta 计算)                         │
  │   compute_delta: +14GB 临时 (current bf16) → 立即释放              │
  │   delta IPC shared: ~200MB                                        │
  │   Actor offload weights: 14GB → CPU                               │
  │   SGLang resume weights: 14GB (从 CPU backup)                     │
  │   SGLang scatter delta: in-place, 0 额外                          │
  │   SGLang resume kv_cache: ~55GB (新分配，无内容)                    │
  │   total peak: max(14GB+14GB, 14GB+200MB+14GB) = ~28GB             │
  │   vs 全量: max(14GB+14GB, 14GB+14GB+14GB) = ~42GB                 │
  └──────────────────────────────────────────────────────────────────┘

CPU 占用:
  Actor optimizer offloaded: 56GB pinned
  Actor weights offloaded: 14GB pinned  
  SGLang weights TMS backup: 14GB (pinned by TMS)
  Delta detector snapshot: 14GB pinned
  Total CPU: ~98GB (单节点通常 512GB+, 可接受)
```

**Delta 的显存收益**：weight sync 阶段峰值从 ~42GB 降到 ~28GB。省的 14GB 是 IPC shared buffer（全量需要分配整个模型大小的共享内存，delta 只需 200MB）。
...
- **要求 `TMS_INIT_ENABLE_CPU_BACKUP=1`**（resume 后有旧权重可打补丁）
- 自动检测 TMS CPU backup 配置，未开启则 fallback 全量
- NCCL reshard 仍用 full tensor（apply delta 后的最新权重）
...
| indices int32 溢出 | 单参数 numel < 2^31，添加 assert 检查 |
| delta 计算 GPU 峰值 | 分批处理，每次只对比一小组参数 |
| NCCL reshard 与 delta 不兼容 | 设计中 NCCL reshard 始终用 full tensor |
| **共卡: TMS 无 CPU backup 导致 delta 失效** | **检测 `TMS_INIT_ENABLE_CPU_BACKUP` 环境变量，未开启则 fallback 全量** |
| **共卡: resume 后旧权重与 delta base 版本不一致** | **detector snapshot 和 TMS backup 来源一致（都是上步 apply 后的结果）** |
| **共卡: scatter_ 后 TMS 不知道权重已更新** | **scatter_ 是 in-place 操作，不改变 storage，TMS 追踪 storage 不受影响** |
| **共卡: kv_cache 无 CPU backup（每步释放重建）** | **kv_cache 不参与 delta（每步 flush 重建是正确的，因为权重变了 cache 就无效）** |
| **共卡: delta detector snapshot + TMS backup 双倍 CPU 内存** | **7B: 14GB×2=28GB；100B/TP8: 12.5GB×2=25GB。单节点 512GB+ 可接受** |
