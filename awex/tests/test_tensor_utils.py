...
class TestGroupTensorsByShapeAndDtype:
    ...
    def test_single_tensor(self):
        ...
        # Groups are flattened because grouping is by dtype only.
    ...
    def test_tensors_different_shapes(self):
        """Test function with tensors of different shapes and one dtype."""
        tensor1 = torch.randn(2, 3, dtype=torch.float32)
        ...
        assert len(result_groups) == 1  # Grouping is by dtype only
        assert len(result_metadata) == 3

        shapes = {
            metadata["original_index"]: metadata["shape"]
